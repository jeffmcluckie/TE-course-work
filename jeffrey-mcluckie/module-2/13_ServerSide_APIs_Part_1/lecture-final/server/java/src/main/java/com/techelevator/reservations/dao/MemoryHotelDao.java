...
public class MemoryHotelDao implements HotelDao {
    ...
    @Override
    public List<Hotel> listByState(String state, String city) {
        List<Hotel> results = new ArrayList<>();
        for (Hotel current : hotels) {
            if (current.getAddress().getState().equalsIgnoreCase(state)){
                // If city is null add, otherwise check for city match before adding
                if (city == null || current.getAddress().getCity().equalsIgnoreCase(city)) {
                    results.add(current);
                }
            }
        }
