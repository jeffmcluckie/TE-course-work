...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
      /* Updating method use the service to save the form data and make a new board */
      boardsService.addBoard(this.newBoard)
        .then( response => {
          if (response.status === 201) {
            ...
            // Get the boards data again (refresh the view) 
            // THIS IS IMPORTANT - Don't just try to update the view with the data you have
            this.retrieveBoards();
          }
        })
      .catch( error => {
        if (error.response) {
          this.errorMsg =
            "Error adding a new board. Response received was '" +
            error.response.statusText +
            "'.";
        } else if (error.request) {
          this.errorMsg =
            "Error adding a new board. Server could not be reached.";
        } else {
          this.errorMsg =
            "Error adding a new board. Request could not be created.";
        }
